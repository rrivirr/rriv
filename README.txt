...
Current columns.txt:
duuid,uuid,time.s,time.h,battery.V,sensor.V,data3,data4,data5,data6,conductivity.mS
...
Use a severed development board to program STM32F103RB via usb:
Remove both ST-LINK jumpers
Connect using jumper wires[severed board -> main board]:
SWD 2 -> CN7 PA14 [SWD clock] (labeled as 2 on programming header)
SWD 3 -> CN7 GND [ground] (labeled as 3 on programming header)
SWD 4 -> CN7 PA13 [SWD data input/output] (labeled as 4 on programming header)
CN3 TX -> CN10 RX
CN3 RX -> CN10 TX

Use a serial monitor @ baud rate 115200 [if "monitor_speed = 115200" is included in the platformio.ini this will not be necessary for vscode]
You may need to restart the nucleo board by hitting the Black reset button to see beginning setup output
If the output gets stuck at 'scanning...' then you will need to power cycle the waterbear shield, this can be done by unplugging the power pack then plugging it back in.
If still stuck, there's a line in setup() called i2c_bus_reset(I2C1); that might help, uncomment, rebuild, reupload and wait for it to run. you won't see any output for a moment or any lights on. I also found that unconnecting the USB along with the power helped. If still stuck and you see a blue light on the Atlas Scientific OEM Development board, you may need to let it sit for a period, with power still plugged in, and then reset again.
...
You should only see serial output from initial setup steps (if reset/first start)
1. Use serial command:
	>WT_CONFIG<
Which will queue to enter config mode. The serial output should immediately change to conductivity readings upon entering in the command.
...
These commands are based off of Atlas Scientific's EC_EZO_Datasheet pg 13/74 two point calibration theory for K 1.0
2. Have the probe in open air and let it stabilize at 0
	>CAL_DRY<
Any output should change to 0 at this point
3. Place probe in low calibration solution, and wait for readings to stabilize. You may want to shake the probe a little to dislodge any air bubbles that may be in the sensing area.
	>CAL_LOW:12880<
The readings will not change at this point.
4. Clean probe (I rinse in water and dry to prevent contamination of calibration fluids), then place into High calibration solution, again shaking it to dislodge air bubbles, and waiting for readings to stabilize.
	>CAL_HIGH:80000<
...
6. Reset to reenter normal operations or use command:
	>WT_CLEAR_MODES<

TO CONFIGURE TIME(Real Time Clock - DS3231):
1. Enter debug values mode via serial command:
	>WT_DEBUG_VALUES<
Which will begin to print logged data values 0 - 10
2. Enter serial command:
	>WT_SET_RTC:[insert epoch time]<
example: >WT_SET_RTC:1605578020<
Change should be immediate or after the second output line, human readable time will also change to reflect this.
I use https://www.epochconverter.com/ and add a few seconds to try to maintain accuracy of timestamp, given how long it takes to copy the command
3. (optional) Enter serial command to clear output:
	>WT_CLEAR_MODES<
...
Which will begin to print logged data values 0 - 10
2. Enter serial command:
	>WT_DEPLOY:[25 characters exact]<
examples:
>WT_DEPLOY:0123456789012345678901234<
>WT_DEPLOY:__bench_tester_01_vDeploy<
>WT_DEPLOY:__fridge_#01_vOEM_EC_DBG_<
Change should be immediate or after the second output line.
3. (optional) Enter serial command to clear output:
	>WT_CLEAR_MODES<
