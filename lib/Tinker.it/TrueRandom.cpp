...
static int TrueRandomClass::randomBitRaw(void) {
...
static int TrueRandomClass::randomBitRaw2(void) {
...
static int TrueRandomClass::randomBit(void) {
  // Software whiten bits using Von Neumann algorithm
  //
  // von Neumann, John (1951). "Various techniques used in connection
  // with random digits". National Bureau of Standards Applied Math Series
...
static char TrueRandomClass::randomByte(void) {
  char result;
  uint8_t i;
  result = 0;
...
static int TrueRandomClass::rand() {
  int result;
  uint8_t i;
  result = 0;
...
static long TrueRandomClass::random() {
  long result;
  uint8_t i;
  result = 0;
...
static long TrueRandomClass::random(long howBig) {
  long randomValue;
  long maxRandomValue;
  long topBit;
  long bitPosition;
...
static long TrueRandomClass::random(long howSmall, long howBig) {
  if (howSmall >= howBig) return howSmall;
  long diff = howBig - howSmall;
  return TrueRandomClass::random(diff) + howSmall;
}

static void TrueRandomClass::memfill(char* location, int size) {
  for (;size--;) *location++ = randomByte();
}

static void TrueRandomClass::mac(uint8_t* macLocation) {
  memfill((char*)macLocation,6);
}

static void TrueRandomClass::uuid(uint8_t* uuidLocation) {
